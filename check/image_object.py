...
class ImageObject(object):
    ...
    def __init__(self, input_path, image_name, output_directory, block_dimension):
        ...
        if self.image_data.mode != 'L':  # L means grayscale
            ...
            self.image_grayscale = self.image_data.convert(
                'L')  # creates a grayscale version of current image to be used later
            grayscale_image_pixels = self.image_grayscale.load()

            # Now this portion will deal with the image height, width and different RGB color scheme
            for y_coordinate in range(0, self.image_height):  # Find coordinate of the image height
                for x_coordinate in range(0, self.image_width):  # Find coordinate of the image width
        ...
        self.features_container = check.container.Container()
        self.block_pair_container = check.container.Container()
        ...
        if self.is_rgb_image:
            for i in tqdm.tqdm(range(0, image_width_overlap + 1, 1)):
                for j in range(0, image_height_overlap + 1, 1):
                    image_block_rgb = self.image_data.crop((i, j, i + self.block_dimension, j + self.block_dimension))#Here segmentation is implemented
                    image_block_grayscale = self.image_grayscale.crop(
                        (i, j, i + self.block_dimension, j + self.block_dimension))
                    image_block = check.block.Blocks(image_block_grayscale, image_block_rgb, i, j,
                                                     self.block_dimension)
                    self.features_container.append_block(image_block.compute_block())
        else:
            for i in range(image_width_overlap + 1): #Overlaping is check here
                for j in range(image_height_overlap + 1):
                    image_block_grayscale = self.image_data.crop(
                        (i, j, i + self.block_dimension, j + self.block_dimension))
                    image_block = check.block.Blocks(image_block_grayscale, None, i, j, self.block_dimension)
                    self.features_container.append_block(image_block.compute_block())
                                    ...
                                    if abs(i_feature[6] - j_feature[6]) < self.P[6]:
                                        if abs(i_feature[0] - j_feature[0]) + abs(i_feature[1] - j_feature[1]) + \
                                            ...
                                            if abs(i_feature[3] - j_feature[3]) + abs(i_feature[4] - j_feature[4]) + \
                                                ...
                                                magnitude = np.sqrt(math.pow(offset[0], 2) + math.pow(offset[1], 2)) #Euclidean distance use here
    ...
    def reconstruct(self):
        ...
        for key in sorted_offset:
            if self.offset_dictionary[key].__len__() < self.Nf * 2:
                break
            # Generate mask of image on the base of store feature in offset directory
