...
class Blocks(object):
    ...
    def compute_pca(self, precision):
        ...
        if self.is_image_rgb:
            ...
            red_feature = image_array[:, :, 0] #red color
            green_feature = image_array[:, :, 1] #green color
            blue_feature = image_array[:, :, 2] #blue color

            concatenated_array = np.concatenate((red_feature, np.concatenate((green_feature, blue_feature), axis=0)),
                                                axis=0)
    ...
    def compute_characteristic_features(self, precision):
        ...
        for y_coordinate in range(0,
                                  self.block_dimension):  # compute the part 1 and part 2 of each feature characteristic
            for x_coordinate in range(0, self.block_dimension):
                # compute c4
